 ...
 * @author  Wesley Kerr
 */
public class FinalLocalPanel extends JPanel {
    
    /** Creates a new instance of DisplayPanel */
    public FinalLocalPanel() {
        super(true);
        init();
    }
    ...
    public void paintRobots(Graphics2D g, int size, Calculator c) {
        ...
        double Avy = -c.finalLocalAvy;
        double Bvy = -c.finalLocalBvy;
        
        g.setColor(Color.black);
        g.fillOval(centerx*size-3, centery*size-3, 6, 6);
        
        g.setColor(Color.blue);
        g.drawLine(centerx*size, centery*size, (int) ((centerx+c.finalLocalAvx)*size), (int) ((centery+Avy)*size));
        
        g.setColor(Color.red);
        g.drawLine(centerx*size, centery*size, (int) ((centerx+c.finalLocalBvx)*size), (int) ((centery+Bvy)*size));
    }
    ...
    public void paintMomentum(Graphics2D g, Calculator c) {
        ...
        g.setColor(Color.black);
        g.drawString("Avx: " + nf.format(c.finalLocalAvx), getWidth()-80, 10);
        g.drawString("Avy: " + nf.format(c.finalLocalAvy), getWidth()-80, 25);
        g.drawString("Bvx: " + nf.format(c.finalLocalBvx), getWidth()-80, 60);
        g.drawString("Bvy: " + nf.format(c.finalLocalBvy), getWidth()-80, 75);
